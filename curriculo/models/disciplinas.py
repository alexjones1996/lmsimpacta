...
class Disciplina(models.Model):
    ...
    def clean(self):
        if self.percentual_teorico > 100 or self.percentual_teorico < 0:
            raise ValidationError('O percentual teórico deve está entre 0 e 100')     
        if self.percentual_pratico > 100 or self.percentual_pratico < 0:
            raise ValidationError('O percentual pratico deve está entre 0 e 100')
        if self.percentual_teorico + self.percentual_pratico != 100:
            raise ValidationError('A soma do percentuais deve ser 100')
